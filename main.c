...
typedef struct node {//node를 struct로 선언
	int key; 
	struct node *left; 
	struct node *right; 
} Node; //구조체 별명 Node
...
int main()
{
	...
	Node* ptr = NULL;	/* temp */ //Node에 대한 포인터 ptr 선언
	...
	do{
		...
		switch(command) {//switch 조건문
		case 'z': case 'Z'://z를 입력받았을 때
			initializeBST(&head); //head의 주소를 매개변수로 하는 initializeBST 함수를 호출하고 메모리를 할당해줌 
			break;
		case 'q': case 'Q'://q를 입력받았을 때
			freeBST(head); //head에 할당된 메모리를 해제하는 함수 호출
			break;
		case 'n': case 'N'://n를 입력받았을 때
			printf("Your Key = ");
			scanf("%d", &key);
			insert(head, key); //노드를 추가하는 insert 함수를 호출
			break;
		case 'd': case 'D':  //d를 입력받았을 때
			printf("Your Key = ");
			scanf("%d", &key);
			deleteLeafNode(head, key);//단말 노드를 삭제하는 함수 deleteLeafNode 함수 호출
			break;
		case 'f': case 'F'://f를 입력받았을 때
			printf("Your Key = ");
			scanf("%d", &key);
	...
	}while(command != 'q' && command != 'Q');//q가 입력되지 않았다면 반복
...
int initializeBST(Node** h) {
	...
	/* create a head node */
	*h = (Node*)malloc(sizeof(Node)); //포인터 h에 메모리 할당
	(*h)->left = NULL;	/* root */ //포인터 h의 left 생성
...
int insert(Node* head, int key)
{
	...
	//Node 포인터 newNode에 메모리할당
	newNode->key = key;//key자리에 입력받은 값 저장
	...
	Node* parentNode = NULL;//Node의 포인터 parentNode 선언
	while(ptr != NULL) {//ptr의 값이 존재한다면

		/* if there is a node for the key, then just return */
		if(ptr->key == key) return 1; //ptr의 key가 입력받은 key와 같다면 1 반환
